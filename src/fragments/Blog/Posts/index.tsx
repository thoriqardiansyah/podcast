...
const Posts: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-2 gap-5 mx-140 mt-100">
      {datas.map((data) => {
        return (
          <div
            className="p-4 border-2 bg-white border-black rounded-lg group hover:shadow-[10px_10px_0_0_black] duration-300 ease-linear transition-all"
            key={data.id}
          >
            <img src={data.img} alt="blog" className="mb-10" />
            <div className="grid gap-y-5">
              <h5 className="font-bold text-sm">{data.category}</h5>
              <h3 className="text-[36.65px] font-bold group-hover:text-secondary duration-300 ease-linear transition-all">
                {data.title}
              </h3>
